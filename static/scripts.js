...
var onDragStart = function (source, piece, position, orientation) {
  ...
  // console.log(`On Drag Start : source = ${source}, piece = ${piece}, position = ${position}, orientation = ${orientation} `);
  if (
    game.game_over() === true ||
...
var getResponseMove = function () {
  ...
  var moves =  game.pgn().split(" ")
  // pgn = game.pgn().split(" ")[1];
  // console.log("Game pgn:",moves[moves.length-1]);
  console.log("===>",startPosition+moves[moves.length-1]);
  pgn =  startPosition+moves[moves.length-1]
  $.get($SCRIPT_ROOT + "/move/"+ pgn, function (data) {
    console.log(data,"*****************");
    game.move(data, { sloppy: true });
    updateStatus();
